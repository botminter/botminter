...
**Supervised mode (GitHub comment-based)** — human gates only at `po:design-review`,
`po:plan-review`, and `po:accept`. The agent adds a review request comment on the
issue and checks for a human response comment on each scan cycle. If no response is
found, the issue stays at its review status — the agent NEVER auto-approves. All
other transitions auto-advance without HIL. All issue operations use the `gh` skill.
