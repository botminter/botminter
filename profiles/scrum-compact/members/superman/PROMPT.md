...
**SUPERVISED MODE: ENABLED (GitHub Comment-Based)**
...
At these gates, the `po_reviewer` hat:
1. Checks if a review request comment already exists on the issue
2. If not, adds ONE review request comment summarizing the artifact
3. Scans for a human response comment (containing `Approved`/`LGTM` or `Rejected: <feedback>`)
4. If human approved → advances the status
5. If human rejected → reverts the status and appends feedback
6. If no human response → returns control. The issue stays at its review status.

**NEVER auto-approve.** If no human response comment is found, take NO action
on the review gate. The issue will be re-checked on the next scan cycle.
